...
| Upstream submodule | `prompts/upstream/research-agent`, `prompts/upstream.toml` | pinned research-agent kernel revision |
...
Shared metaprompts are pinned as the Git submodule
`prompts/upstream/research-agent` from
...
For a normal refresh after updating the submodule pointer, run:
...
The local wrapper reads the pinned submodule, writes `prompts/upstream.toml`,
and verifies that `prompts/meta/kernel-project.md` keeps the same SHA-256 before
and after sync. The upstream sync materializes only shared `kernel-*.md` files
into `prompts/meta/` and writes `prompts/upstream-managed.json`. If sync writes
`prompts/REDEPLOY_REQUIRED.md`, keep it until local deployment docs, skills,
helper-script policy, prompt counts, Q3-AUDIT, wiki-packet checks, and Q3b token telemetry pass;
