...
| RA-PR-003 | High | Notation is inconsistent. The same `\tau` role is used for sequence index and threshold; `B` can mean bucket count or batch size; `N`, `D`, frame, sequence, and score notation are not defined once and reused consistently. | Add a notation paragraph or table. Use a non-conflicting threshold symbol such as `\gamma`, reserve `k` or `i` for frame time, and distinguish batch size from bucket count. | RESOLVED |
| RA-PR-004 | High | The feature schema has a hidden dimensional mismatch. The text moves between 38 cheap metadata features, optional entropy/compression, and `D=40` padded examples without making the profiles explicit. | Define fixed feature profiles: metadata-only, metadata plus cheap telemetry, and padded implementation width. Tie all model examples to the padded `D=40` contract while keeping metadata-only as a required ablation. | RESOLVED |
...
- Introduced consistent notation for frames, sequences, feature groups, batch
  size, histogram bucket count, score, and threshold.
- Made feature profiles explicit: metadata-only P0, sequentiality P1, and
  padded telemetry P2, with unavailable optional slots zero-filled and masked
  from loss/score.
