...
再構成誤差が、write-heavy burst、LBA 分布変化、I/O 長分布変化、
read/write 比率変化、圧縮率またはエントロピー相当信号の変化を伴う
攻撃窓で上昇し、単純な write/entropy ルールを超える情報を持つかもしれない、
というものである。AE は既定解ではなく、メモリ制約付き仮説として扱う。
入力は 10 秒フレームを $N$ 個束ねた $N \times 10$ 秒の時系列であり、
最終実装では Alibaba MNN による CPU 推論を想定する。モデル経路に
使えるメモリは MNN ランタイムを除いて 500 KB とし、性能、MNN 変換、
score parity、peak model-owned memory は、manifest 付き実験と測定が
揃うまで非主張とする。
\end{abstract}
...
\begin{table}[tb]
  ...
  \begin{tabularx}{\linewidth}{lYY}
    ...
    非主張。主要評価項目。 \\
    device-fit claim &
    MNN 変換済み model、offline/MNN score parity、peak model-owned memory 測定。 &
    非主張。500 KB は制約条件。 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
評価する動機になるが、特定装置で 500 KB のモデル経路メモリが確保できる
証拠にはならない。
...
しかし、ストレージ装置内、10 秒統計、500 KB モデル制約、MNN CPU 推論という
...
本稿のモデル例では padding を含む $D=40$ を使う。特徴群の集合を $G$、
特徴群 $g$ に属する index 集合を $I_g$、AE の再構成出力を
$\hat{X}_k=f_{\theta}(X_k)$、anomaly score を $s(X_k)$、alert threshold を
$\gamma$ とする。bucket 数は $B_{\mathrm{hist}}$、training batch size は
$B_{\mathrm{batch}}$ と書き分ける。
...
\begin{align}
  x_k = \operatorname{concat}(&n^r_k, n^w_k, b^r_k, b^w_k,
         h^{r,\mathrm{LBA}}_{k,1:8}, h^{w,\mathrm{LBA}}_{k,1:8},
         h^{r,\mathrm{len}}_{k,1:8}, h^{w,\mathrm{len}}_{k,1:8}, \nonumber \\
        &q^r_k, q^w_k, c_k, p_k) ,
  \label{eq:frame}
\end{align}

式 \eqref{eq:frame} の comma-separated entry はすべて scalar ではなく、
12 個の feature group を連結している。ここで、$n$ は I/O 数、$b$ はバイト数、
$h_{\cdot,1:8}$ は 8 bucket の固定ヒストグラム、
$q$ は sequentiality 近似、$c$ は optional な圧縮率または entropy 相当信号、
$p$ は固定幅 alignment のための padding slot である。したがって次元は
P0 で $D=4+4 \times 8=36$、P1 で $D=36+2=38$、P2 で
$D=38+1+1=40$ となる。AE は単一フレーム $x_k$ ではなく、
...
\begin{table}[tb]
  ...
  \begin{tabularx}{\linewidth}{lrlY}
    ...
    特徴群 & 次元 & profile & 備考 \\
    \midrule
    read/write count & 2 & P0 & log scaling または robust scaling。 \\
    read/write bytes & 2 & P0 & intensity と anomaly の混同に注意。 \\
    read LBA histogram & 8 & P0 & namespace size で正規化。 \\
    write LBA histogram & 8 & P0 & high bits または shift bucket。 \\
    read length histogram & 8 & P0 & log2 近似 bucket。 \\
    write length histogram & 8 & P0 & log2 近似 bucket。 \\
    sequentiality counters & 2 & P1 & 直前 I/O との比較のみ。 \\
    compression/entropy-like signal & 0--1 & P2 & 既存装置カウンタがある場合のみ。 \\
    padding/alignment & 0--1 & P2 & MNN 固定 shape 用。score から除外する。 \\
    \bottomrule
  \end{tabularx}
\end{table}

ここで 4 本の histogram とは、
\emph{read LBA histogram}、\emph{write LBA histogram}、
\emph{read length histogram}、\emph{write length histogram} である。
たとえば write LBA histogram は、その 10 秒窓で発生した write が
namespace 上のどの LBA bucket に落ちたかを 8 bin で数えたもの、
write length histogram は write の転送長を log2 近似の 8 bin で数えたものである。
read 側も同じ作り方で別に集計するため、$4 \times 8=32$ 次元になる。
P0 は metadata-only profile であり、count/bytes とこの 4 本の 8 bucket histogram だけの
$D=36$ である。P1 は安価な sequentiality を加えた $D=38$、P2 は cheap telemetry
slot と padding を加えた実装例 $D=40$ である。RanSAP の write entropy は
P2 の slot に入れて upper-bound または ablation として評価できるが、
target device が安価なカウンタを持つことを確認するまで、組み込み主張の
必須特徴にしてはならない。
MNN 固定 shape の実装では $D=40$ を使い、利用できない optional slot は
...
scalar は MSE または Huber loss を使う。しきい値 $\gamma$ は benign calibration split
...
\begin{table}[tb]
  ...
  \begin{tabular}{llrrl}
    ...
    \midrule
    AE-0 & \texttt{480-32-8-32-480} & 31,784 & 124 KB & flat baseline \\
    AE-1 & \texttt{480-64-16-4-16-64-480} & 64,260 & 251 KB & stronger flat AE \\
    AE-2 & shared frame + sequence bottleneck & 12,540 & 49 KB & deployable first pick \\
    AE-3a & GRU seq2seq, hidden 24 & 9,352 & 37 KB & first recurrent AE \\
    AE-3b & GRU seq2seq, hidden 32 & 14,760 & 58 KB & stronger recurrent AE \\
    AE-4 & temporal Conv1D AE, 24 channels & 9,744 & 38 KB & MNN-friendly temporal AE \\
    AE-5 & grouped bucket CNN + GRU hidden 24 & 12,792 & 50 KB & constrained CNN-GRU test \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
\begin{longtable}{@{}p{0.08\linewidth}p{0.45\linewidth}p{0.24\linewidth}r@{}}
  ...
  \bottomrule
  \endlastfoot
  AE-0 & \texttt{Input(shape=(12, 40))} & \texttt{(None, 12, 40)} & 0 \\
  AE-0 & \texttt{Flatten()} & \texttt{(None, 480)} & 0 \\
  AE-0 & \texttt{Dense(32, activation="relu")} & \texttt{(None, 32)} & 15,392 \\
  AE-0 & \texttt{Dense(8, activation="relu", name="z")} & \texttt{(None, 8)} & 264 \\
  AE-0 & \texttt{Dense(32, activation="relu")} & \texttt{(None, 32)} & 288 \\
  AE-0 & \texttt{Dense(480, activation="linear")} & \texttt{(None, 480)} & 15,840 \\
  AE-0 & \texttt{Reshape((12, 40))} & \texttt{(None, 12, 40)} & 0 \\
  \multicolumn{3}{r}{AE-0 total} & 31,784 \\
  \midrule
  AE-2 & \texttt{Input(shape=(12, 40))} & \texttt{(None, 12, 40)} & 0 \\
  AE-2 & \texttt{TD(Dense(24, activation="relu"))} & \texttt{(None, 12, 24)} & 984 \\
  ...
  AE-2 & \texttt{TD(Dense(40, activation="linear"))} & \texttt{(None, 12, 40)} & 1,000 \\
  \multicolumn{3}{r}{AE-2 total} & 12,540 \\
  \midrule
  AE-3a & \texttt{Input(shape=(12, 40))} & \texttt{(None, 12, 40)} & 0 \\
  AE-3a & \texttt{GRU(24, return\_sequences=False)} & \texttt{(None, 24)} & 4,752 \\
  AE-3a & \texttt{RepeatVector(12)} & \texttt{(None, 12, 24)} & 0 \\
  AE-3a & \texttt{GRU(24, return\_sequences=True)} & \texttt{(None, 12, 24)} & 3,600 \\
  AE-3a & \texttt{TD(Dense(40, activation="linear"))} & \texttt{(None, 12, 40)} & 1,000 \\
  \multicolumn{3}{r}{AE-3a total} & 9,352 \\
  \midrule
  AE-4 & \texttt{Input(shape=(12, 40))} & \texttt{(None, 12, 40)} & 0 \\
  AE-4 & \texttt{Conv1D(24, 3, padding="same", activation="relu")} & \texttt{(None, 12, 24)} & 2,904 \\
  ...
  AE-4 & \texttt{Conv1D(24, 3, padding="same", activation="relu")} & \texttt{(None, 12, 24)} & 1,752 \\
  AE-4 & \texttt{Conv1D(40, 3, padding="same", activation="linear")} & \texttt{(None, 12, 40)} & 2,920 \\
  \multicolumn{3}{r}{AE-4 total} & 9,744 \\
  \midrule
  AE-5 & \texttt{Input(shape=(12, 40))} & \texttt{(None, 12, 40)} & 0 \\
  AE-5 & \texttt{Reshape((12, 5, 8, 1))} & \texttt{(None, 12, 5, 8, 1)} & 0 \\
  AE-5 & \texttt{TD(TD(Conv1D(8, 3, padding="same", activation="relu")))} & \texttt{(None, 12, 5, 8, 8)} & 32 \\
  AE-5 & \texttt{TD(Flatten())} & \texttt{(None, 12, 320)} & 0 \\
  AE-5 & \texttt{TD(Dense(16, activation="relu"))} & \texttt{(None, 12, 16)} & 5,136 \\
  ...
  AE-5 & \texttt{TD(Dense(40, activation="linear"))} & \texttt{(None, 12, 40)} & 1,000 \\
  \multicolumn{3}{r}{AE-5 total} & 12,792 \\
\end{longtable}
\setlength{\tabcolsep}{6pt}
\normalsize

AE-5 は 500 KB 制約内で CNN-GRU 仮説を試す最小構成として、以下の制約を置く。
feature layout は $D=40=5 \times 8$ に固定し、4 つの histogram family と
1 つの scalar/padding family を同じ bucket 幅に整列する。CNN は
各 family の 8 bucket 方向に共有 Conv1D を 1 層だけ適用し、filters は 8、
kernel は 3 とする。GRU は encoder/decoder とも 1 層、hidden size 24 に固定し、
decoder は \texttt{RepeatVector(12)} から系列を再生成する。Conv2D、
transposed convolution、多層 GRU、attention、モデル内 thresholding は使わない。
この条件では weight は約 50 KB に収まるが、MNN operator workspace と tensor
...
\begin{itemize}
  \item Rule baselines: write ratio threshold、write bytes または write IOPS の
        rolling z-score、LBA coverage または LBA histogram shift、cheap telemetry が
  ...
  \item Supervised reference models: 小型 decision tree、random forest、XGBoost 相当。
        ...
        deployment claim と同じ条件の baseline ではない。
\end{itemize}
...
\begin{enumerate}
  ...
  \item peak が 500 KB を超える場合、bucket 数、系列長 $N$、hidden size、
...
read/write counter、read/write LBA histogram、read/write length histogram、
sequentiality、optional compression/padding を同じ index に保つ。ONNX/MNN export は
