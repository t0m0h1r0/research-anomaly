...
エントロピー相当信号を観測する。提案候補は、10 秒単位の統計列を入力とする
...
\begin{enumerate}
  \item ブロックストレージから安価に得られる 10 秒統計だけで、
...
最初の実装では LBA と長さの bucket 数をそれぞれ 8 とし、
sequence length $T=12$、すなわち 2 分の文脈を用いる。
...
長さ $T$ の入力列を $X=(x_{t-T+1},\ldots,x_t)$ とし、AE を
...
\begin{figure}[tb]
  ...
  \begin{tikzpicture}[
    ...
    \node[box, right=of stats] (seq) {$T=12$\\sequence};
...
\begin{table}[tb]
  ...
  \begin{tabularx}{\linewidth}{lY}
    ...
    $T=6,12,24$ & 文脈長と 500KB 制約の trade-off。 \\
...
\begin{enumerate}
  \item 入力 tensor shape と 10 秒統計スキーマを固定する。
  ...
  \item peak が 500KB を超える場合、bucket 数、sequence length、hidden size、
...
10 秒統計を正常時系列として学習し、AE の再構成誤差上昇をランサムウェアらしい
