...
\begin{align}
  x_k = [&n^r_k, n^w_k, b^r_k, b^w_k,
         h^{r,\mathrm{LBA}}_k, h^{w,\mathrm{LBA}}_k,
         h^{r,\mathrm{len}}_k, h^{w,\mathrm{len}}_k, \nonumber \\
        &q^r_k, q^w_k, c_k, p_k] ,
  \label{eq:frame}
\end{align}
...
$p$ は固定幅 alignment のための padding slot である。AE は単一フレーム $x_k$ ではなく、
...
P0 は metadata-only profile であり、count/bytes と LBA/length histogram だけの
