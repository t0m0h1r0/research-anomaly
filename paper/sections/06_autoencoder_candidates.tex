...
小型候補との比較が必須である。
本稿では operator の役割を固定する。Dense は frame denoising または情報圧縮、GRU は各 frame への
時系列文脈付与を担う。Conv1D は圧縮ではなく feature analysis のために使い、
AE-4 では $K=3$ で局所時間 pattern を抽出し、AE-5 では $K=1$ で同一 frame 内の
異種 scalar feature を混合する。したがって、GRU や Conv1D に圧縮責任を曖昧に
...
各 frame に付与するために使う。AE-4 の Conv1D は局所時間特徴を多チャネルに
展開して抽出するために使い、AE-5 の $K=1$ Conv1D は同一 frame 内の異種 feature
を混合するために使う。いずれも、情報を減らす処理は TimeDistributed Dense
bottleneck に明示的に任せる。
