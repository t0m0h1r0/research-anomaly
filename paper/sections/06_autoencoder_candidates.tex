...
volume あたり 500 KB という制約、さらに多数 volume へ同じ検出器を展開する制約では、
小型候補との比較が必須である。
...
\begin{table}[tb]
  ...
  \begin{tabularx}{\linewidth}{lY Y}
    ...
    最も複雑で、volume あたり 500 KB と MNN operator support の制約を受けやすい。 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
flattened AE の最低線である。AE-2 と AE-4 は volume あたり 500 KB 制約下で
最も実装しやすく、多数 volume へ複製した場合にも総コストを抑えやすい候補である。
AE-3a は recurrent operator が MNN 上で許容できるかを測る候補である。
...
AE-5 は volume あたり 500 KB 制約内で CNN-GRU 仮説を試す最小構成として、以下の制約を置く。
...
buffer を含む per-volume 500 KB 達成と aggregate device-fit は変換後の実測で判定する。
...
\subsection{volume あたり 500 KB モデルメモリ予算}

本研究では、MNN ランタイム自体は volume あたり 500 KB 予算から除外する。ただし、モデルファイルが
...
この配分は、volume あたりのモデル経路メモリを抑えるための仮配分であり、
最終判断は MNN 変換後の実測に基づく。
...
volume ごとに複製され得る。したがって実験では、単一 volume の peak だけでなく、
$V$ を変えた aggregate memory と 10 秒あたり CPU 時間を報告する。
特に $N$ は、input/output sequence buffer と recurrent/temporal operator の
intermediate tensor に直接影響するため、検出性能だけでなく volume あたり 500 KB 予算との
trade-off として選ぶ。
